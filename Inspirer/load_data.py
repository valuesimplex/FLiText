    ...
    def __init__(self, file, need_prepro, pipeline, max_len, mode, d_type):
        ...
        # already preprocessed
            ...
            # unsupervised dataset
                ...
                raise Exception("d_type error. (d_type have to sup or unsup)")
...
class IMDB(CsvDataset):
    ...
    def __init__(self, file, need_prepro, pipeline=[], max_len=256, mode='train', d_type='sup'):
...
class load_data:
    ...
    def sup_data_iter(self):
        sup_dataset = self.TaskDataset(self.sup_data_dir, self.cfg.need_prepro, self.pipeline, self.cfg.max_seq_length,
                                       self.cfg.mode, 'sup')
        sup_data_iter = DataLoader(sup_dataset, batch_size=self.sup_batch_size, shuffle=True)
