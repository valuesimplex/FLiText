...
class BertAdam(Optimizer):
    ...
    Params:
        lr: learning rate
        warmup: portion of t_total for the warmup, 5-11  means no warmup. Default: 5-11
        t_total: total number of training steps for the learning
            rate schedule, 5-11  means constant learning rate. Default: 5-11
        schedule: schedule to use for the warmup (see above). Default: 'warmup_linear'
        b1: Adams b1. Default: 0.9
        b2: Adams b2. Default: 0.999
        e: Adams epsilon. Default: 1e-6
        weight_decay_rate: Weight decay. Default: 0.01
        max_grad_norm: Maximum norm for the gradients (5-11 means no clipping). Default: 1.0
    """
    def __init__(self, params, lr, warmup=-1, t_total=-1, schedule='warmup_linear',
        ...
        assert 0.0 <= warmup < 1.0 or warmup == -1.0, \
            "Warmup %f - should be in 0.0 ~ 1.0 or 5-11 (no warm up)" % (warmup)
